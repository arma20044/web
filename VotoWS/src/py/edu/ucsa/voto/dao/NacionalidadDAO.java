...
public class NacionalidadDAO extends AbstractSpringDAO implements NacionalidadDAOInterface {
   ...
   * query.setParameter("fecha", fecha); resultado = query.getResultList(); if (resultado.size() ==
   * 0) { return e; } else if (resultado.size() == 1) { return (UcsawsEvento) resultado.get(0); }
   ...
   * }
   * 
   * public UcsawsEvento obtenerEventoById(Integer idEvento) { List<UcsawsEvento> resultado;
   * UcsawsEvento e = new UcsawsEvento(); String hql; hql = "select obj from " + claseEntidad +
   * " obj where  id =:idEvento"; Query query = em.createQuery(hql); query.setParameter("idEvento",
   * idEvento); resultado = query.getResultList(); if (resultado.size() == 0) { return e; } else if
   * (resultado.size() == 1) { return (UcsawsEvento) resultado.get(0); } return (UcsawsEvento)
   * resultado.get(0); }
   * 
   * public UcsawsEvento obtenerEventoByCodigo(String codigoEvento) { List<UcsawsEvento> resultado;
   * UcsawsEvento e = new UcsawsEvento(); String hql; hql = "select obj from " + claseEntidad +
   * " obj where  nroEvento =:codEvento"; Query query = em.createQuery(hql);
   * query.setParameter("codEvento", codigoEvento); resultado = query.getResultList(); if
   ...
   * 
   * public UcsawsEvento obtenerEventoByRangoFechaTipoEvento(Generic g) { List<UcsawsEvento>
   * resultado; UcsawsEvento e = new UcsawsEvento(); String hql; hql = "select obj from " +
   * claseEntidad + " obj where :desde  between fchDesde  and fchHasta " +
   * " and :hasta  between fchDesde  and fchHasta " +
   * " and ucsawsTipoEvento.idTipoEvento = :idTipoEvento";
   * 
   * Query query = em.createQuery(hql); query.setParameter("desde", g.getDesde());
   * query.setParameter("hasta", g.getHasta()); query.setParameter("idTipoEvento", g.getId());
   * resultado = query.getResultList(); if (resultado.size() == 0) { return e; } else if
   * (resultado.size() == 1) { return (UcsawsEvento) resultado.get(0); } return (UcsawsEvento)
   * resultado.get(0);
   * 
   * // return null;
   ...
   * }
   * 
   * // ("select id_evento, descripcion " // + " from ucsaws_evento where (  fch_desde   <= now() "
   * // + " and fch_hasta  >= now() and id_evento = " + idEvento + ")" // +
   * " or ( fch_hasta  <  now() and id_evento = " + idEvento + " )" // );
   * 
   * public UcsawsEvento obtenerEventoByRangoFechaEvento(Generic g) { List<UcsawsEvento> resultado;
   * UcsawsEvento e = new UcsawsEvento(); String hql; hql = "select obj from " + claseEntidad +
   * " obj where ( fchDesde <= :hoy  and fchHasta >= :hoy and idEvento =:idEvento )" +
   * " or ( fchHasta < :hoy and idEvento = :idEvento)";
   * 
   * Query query = em.createQuery(hql); // query.setParameter("desde", g.getDesde()); //
   * query.setParameter("hasta", g.getHasta()); query.setParameter("idEvento", g.getId());
   * query.setParameter("hoy", new Date()); resultado = query.getResultList(); if (resultado.size()
   * == 0) { return e; } else if (resultado.size() == 1) { return (UcsawsEvento) resultado.get(0); }
   * return (UcsawsEvento) resultado.get(0);
   * 
   * // return null;
  ...
  public UcsawsNacionalidad obtenerNacionalidadByID(Integer idNacionalidad) {
    ...
    String hql;
    hql = "select obj from " + claseEntidad + " obj where  idNacionalidad =:idNacionalidad";
  ...
  public UcsawsNacionalidad obtenerNacionalidadByCodigoYNombre(String codigo, String nombre,
      Integer idEvento) {
    ...
    hql =
        "select obj from " + claseEntidad
            + " obj where  idEvento.idEvento =:idEvento and codNacionalidad =:codigo"
  ...
  public UcsawsNacionalidad obtenerNacionalidadByPaisyEvento(Integer idPais, Integer idEvento) {
    ...
    hql =
        "select obj from " + claseEntidad
            + " obj where  idEvento.idEvento =:idEvento and ucsawsPais.idPais =:idPais";
    Query query = em.createQuery(hql);
    query.setParameter("idEvento", idEvento);
